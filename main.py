...
def train(opt):
    ...
    logger.info(
        "Total epochs needed: {} for {} iterations".format(total_epoch, total_iter)
    )
...
def profile(opt, lr_size, test_speed=False):
    ...
    logger.info("-" * 40)
    logger.info(
        "Super-resolute data from {}x{}x{} to {}x{}x{}".format(
    ...
    if test_speed:
        ...
        logger.info(
            "Speed (FPS): {:.3f} (averaged for {} runs)".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_dir", type=str, required=True, help="directory of the current experiment"
    )
    parser.add_argument(
